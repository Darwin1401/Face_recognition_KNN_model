...
def predict(img_path, knn_clf=None, model_path=None, threshold=0.6): # 0.6 needs accuracy 40+, 0.4 needs accuracy 60+
...
webcam = cv2.VideoCapture(0) # 0 to use webcam, video path if use videos
while True:
    # Loop until the camera is working
    rval = False
    while (not rval):
        # Put the image from webcam into 'frame'
    ...
    for name, (top, right, bottom, left) in predictions:
        top *= 4 # Scale back the frame since it was scaled do 1/4 size
        right *= 4
        bottom *= 4
        left *= 4
